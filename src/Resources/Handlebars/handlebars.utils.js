...
function _calculateSavedCart(items, totalizer, options) {
  var total = 0;
  items.forEach(function(item){
    if(item.availability != "withoutStock"){
...
function _replaceText(text1, text2, replaceBy, options) {
  return text1.replace(text2, replaceBy);
}

function _getCategoryName(productCategories) {
  var categories = Object.keys(productCategories)
  return productCategories[categories[categories.length - 1]]
}
...
Handlebars.registerHelper('equalZero', function(value) {   return value === 0 });
...
Handlebars.registerHelper('getCategoryName', function(productCategories) {   return _getCategoryName(productCategories); });
Handlebars.registerHelper('savedCart', function(items, total, options) {   return _calculateSavedCart(items, total, options); });
Handlebars.registerHelper('replace', function(text1, text2, replaceBy, options) {   return _replaceText(text1, text2, replaceBy, options); });
